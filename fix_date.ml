...
let is prefix prop =
  if String.prefix prop.name (String.length prefix) = prefix
  then true else false

let time_of_string s =
  let open String.Search_pattern in
  if String.length s = 19 then begin
...
struct
  ...
  let update image p =
    ...
    then begin
      ...
      | Some _, "Image.DateTime"
      | None,   "Image.DateTime" -> Some (
          {
            date_time = time_of_string p.value
          }
        )
      | _, _ -> image
    end else image
...
struct
  ...
  let update photo p =
    ...
    then begin
      ...
      | Some i, "Photo.DateTimeDigitized" -> Some {
          i with
          date_time_digitized = time_of_string p.value
        }
      | Some i, "Photo.DateTimeOriginal" -> Some {
          i with
          date_time_original = time_of_string p.value
        }
      | None, "Photo.DateTimeDigitized" -> Some {
          empty with
          date_time_digitized = time_of_string p.value
        }
      | None, "Photo.DateTimeOriginal" -> Some {
          empty with
          date_time_original = time_of_string p.value
        }
      | _, _ -> photo
...
let exif_of_prop_list fname p_list =
  ...
  List.fold p_list ~init:{Exif.empty with fname}~f:(fun exif p ->
      { exif with
        image = Image.update exif.image p;
        photo = Photo.update exif.photo p;
      }
    )
...
let extract_exiv2 fname =
  try
    ...
    let prop_list = List.fold
        ~init:[]
        output_lines ~f:(
        fun acc line ->
          if String.prefix line 5 = "Exif." then begin
...
let regexp1  = Str.regexp "^.*/\\([1-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]\\)\\.\\(jpg\\|png\\)$"
let regexp2  = Str.regexp "^.*/\\(20[0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\) \\([0-9][0-9]\\)\\.\\([0-9][0-9]\\)\\.\\([0-9][0-9]\\)[-_0-9a-zA-Z]*\\.\\(jpg\\|png\\|mp4\\|jpeg\\|3gp\\|mov\\)$"
let regexp3  = Str.regexp "^.*/video-\\(20[0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)\\.mp4$"
let regexp4  = Str.regexp "^.*/\\(20[0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)_\\([0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)[-_0-9a-zA-Z]*\\.\\(jpg\\|png\\|mp4\\|JPG\\)$"
let regexp5  = Str.regexp "^.*/IMG_\\(20[0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)_\\([0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\).\\(jpg\\|png\\|mp4\\)"
let regexp6  = Str.regexp "^.*/VID_\\(20[0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)_\\([0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\).\\(jpg\\|png\\|mp4\\|3gp\\)"
let regexp7  = Str.regexp "^.*/CameraZOOM-\\(20[0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)[0-9][0-9][0-9]\\.\\(jpg\\|png\\|mp4\\)$"
let regexp8  = Str.regexp "^.*/Screenshot_\\(20[0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)\\.\\(png\\|jpg\\)$"
let regexp9  = Str.regexp "^.*/IMG-\\(20[0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)-W\\(.+\\).jpg$"
let regexp10 = Str.regexp "^.*/VID-\\(20[0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)-W\\(.+\\).mp4$"
let regexp11 = Str.regexp "^.*/\\(20[0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)\\([0-9][0-9]\\)\\([0-9][0-9]\\)\\.webm$"
let regexp12 = Str.regexp "^.*/Screenshot from \\(20[0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\) \\([0-9][0-9]\\):\\([0-9][0-9]\\):\\([0-9][0-9]\\)\\.\\(png\\|jpg\\)$"
let regexp13 = Str.regexp "^.*/Screenshot from \\(20[0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\) \\([0-9][0-9]\\)-\\([0-9][0-9]\\)-\\([0-9][0-9]\\)\\.\\(png\\|jpg\\)$"
...
let extract_group_or_def ?(def=0) group_num str =
  try Str.matched_group group_num str |> int_of_string
  with
...
let action_on_file kind () fname =
  ...
  | Core_unix.S_REG -> begin(
      ...
      | Some exif_dt, Some fn_dt -> begin
          ...
          then begin
            ...
            | Some dt -> begin
                ...
                let fname = match fname' with | None -> fname | Some f -> f in
                set_exif fname dt;
      ...
      | None, None -> begin
          Printf.eprintf "NESSUNA INFO CRONOLOGICA: \"%s\"\n%!" fname
        end);
      if !dry_run
      then Printf.printf "\n%!"
    end
